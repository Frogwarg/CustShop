...
const DesignModeration = () => {
  const [designs, setDesigns] = useState<Design[]>([]);
  const [availableTags, setAvailableTags] = useState<{ id: string; name: string }[]>([]);
  const [loading, setLoading] = useState(true);
  const [formData, setFormData] = useState<FormData>({});
  const [editingField, setEditingField] = useState<{ designId: string; field: 'name' | 'description' | null }>({
    designId: '',
    field: null,
  });
      ...
      {designs.map((design) => (
        <div key={design.id} className={styles.designCard}>
          ...
          <div className={styles.imageWrapper}>
            <Image src={design.previewUrl} alt={design.name} fill className={styles.designImage} />
          </div>
          <input
            type="number"
            placeholder="Цена"
          ...
          <div className={styles.buttonGroup}>
            <button onClick={() => handleDecision(design.id, 'approve')} className={styles.approveButton}>
              Одобрить
            </button>
            <button onClick={() => handleDecision(design.id, 'reject')} className={styles.rejectButton}>
              Отклонить
            </button>
          </div>
        </div>
...
export default DesignModeration;